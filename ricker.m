function [wavelet,tw]=ricker(dt,fdom,tlength,flag)
...
% create a time vector
  ...
  nzero=ceil((n+1)/2);%zero time sample is here.  ceil向上(大)取整
...
% create the wavelet
  pf=pi^2*fdom^2;
  wavelet=(1-2.*pf*tw.^2).*exp(-pf*tw.^2); % wavelet(t)=(1-2*(pi*fdom*t)^2)*exp(-(pi*fdom*t)^2)

% normalize
% generate a refenence sinusoid at the dominant frequency:flag=2
wavelet=wavenorm(wavelet,tw,flag);